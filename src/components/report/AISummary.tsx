...
export default function AISummary({ summary, delay = 1.8 }: AISummaryProps) {
  ...
  return (
    <motion.div
      className="rounded-xl p-6 bg-white border border-warm-gray/20 shadow-sm"
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-deep-slate font-semibold text-sm">AI Risk Assessment</h3>
          <span className="text-[10px] uppercase tracking-widest text-warm-gray/60">
      ...
      {/* Divider */}
      <div className="h-px bg-warm-gray/10 mb-4" />

      {/* Summary text with typewriter */}
      <div className="min-h-[80px]">
        {!started ? (
          <div className="flex items-center gap-2 text-warm-gray/70 text-sm">
            <motion.span
          ...
          <p className="text-deep-slate text-sm leading-relaxed">
            {displayedText}
            {displayedText.length < summary.length && (
              <motion.span
                className="inline-block w-0.5 h-4 bg-sage ml-0.5 align-text-bottom"
                animate={{ opacity: [1, 0] }}
