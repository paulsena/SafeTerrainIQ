    ...
    <PageTransition className="min-h-screen bg-warm-white flex flex-col pt-safe px-safe overflow-y-auto overflow-x-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 pb-16">
        ...
        >
          ...
          <div className="bg-white rounded-xl shadow-card border border-warm-gray/20 p-6">
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {professionals.map((pro, i) => (
              <motion.div
                key={pro.name}
                className="bg-white rounded-xl shadow-card border border-warm-gray/20 p-5 hover:border-sage/30 transition-colors group"
                initial={{ opacity: 0, y: 12 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.3, delay: 0.4 + i * 0.05 }}
              >
                <h3 className="text-deep-slate font-semibold text-sm mb-1 group-hover:text-sage transition-colors">
                  {pro.name}
                </h3>
                ...
                <div className="flex items-center gap-4 text-xs">
                  <a
                    href={`tel:${pro.phone}`}
                    className="flex items-center gap-1.5 text-warm-gray hover:text-deep-slate transition-colors"
                  >
        ...
        >
          <button
            ...
            className="group flex items-center gap-2 px-8 py-3.5 rounded-xl bg-white hover:bg-warm-gray/10 border border-warm-gray/30 hover:border-sage/30 text-deep-slate hover:text-sage font-medium text-sm tracking-wide transition-all duration-200 cursor-pointer"
          >
